...
void MaxSPRPolicy(UCPD_PORT_Number port_number, UCPD_Policy_Event event)
{
	...
	switch (event)
	{
		...
		{
			uint8_t max_spr = 0u;
			while (src_pdo[max_spr].pdo_type == UCPD_PDO_TYPE_FIXED_SUPPLY && max_spr < pdo_count)
				max_spr++;
	...
	switch (event)
	{
		case UCPD_POLICY_EVENT_EVALUATE_CAPABILITIES:
		{
			for (uint8_t i = 0u; i < pdo_count; i++)
			{
				...
				{
					UCPD_POLICY_SelectAPDO(port_number, i + 1u, src_pdo[i].APDO.PPS.min_voltage100mV * 5u,
...
void PPSImmediatePolicyCallback(UCPD_PORT_Number port_number, UCPD_Status status)
{
}
...
void PPSImmediatePolicy(UCPD_PORT_Number port_number, UCPD_Policy_Event event)
{
	...
	switch (event)
	{
		case UCPD_POLICY_EVENT_EVALUATE_CAPABILITIES:
		{
			for (uint8_t i = 0u; i < pdo_count; i++)
			{
				if (src_pdo[i].pdo_type == UCPD_PDO_TYPE_APDO && src_pdo[i].APDO.apdo_type == UCPD_APDO_TYPE_PPS)
				{
					...
					time += 0.05;
					UCPD_POLICY_SelectAPDO(port_number, i + 1u, a, src_pdo[i].APDO.PPS.max_current50mA, false);
...
void EPRPolicy(UCPD_PORT_Number port_number, UCPD_Policy_Event event)
{
	switch (event)
	{
		case UCPD_POLICY_EVENT_EVALUATE_CAPABILITIES:
		{
			...
			{
				UCPD_POLICY_SelectFixedSupply(port_number, 1u, 100u, 100u, false);
				if (UCPD_IsEPRCapable(port_number)) (void)UCPD_EnterEPRAsync(0, 120, EPREnterCallback);
			}
...
void MultipleInflightCallsPolicy(UCPD_PORT_Number port_number, UCPD_Policy_Event event)
{
	switch (event)
	{
		case UCPD_POLICY_EVENT_EVALUATE_CAPABILITIES:
...
void RunExamples(void)
{
	...
	config.policy = EPRPingPongPolicy;
