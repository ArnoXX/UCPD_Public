...
void UCPD_PRL_SM_Delta(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  case PE_PRL_EVENT_PRL_INIT: {
    ...
    UCPD_PRL_SM_CHUNKED_ROUTER_Enter(pe_prl_cad,
                                     PRL_RTR_STATE_WAIT_FOR_MESSAGE_FROM_RRL);
    UCPD_PRL_SM_CHUNKED_RX_Enter(pe_prl_cad,
                                 PRL_RCH_STATE_WAIT_FOR_MESSAGE_FROM_PRL);
    UCPD_PRL_SM_CHUNKED_TX_Enter(
        pe_prl_cad, PRL_TCH_STATE_WAIT_FOR_MESSAGE_REQUEST_FROM_PE);
  ...
  case PE_PRL_EVENT_MSG_TX_COMPLETED: { // done
    UCPD_MSG_HEADER *header = &pe_prl_cad->buffers[UCPD_TX_BUFFER_INDEX].header;

    if (header->message_type == UCPD_GOODCRC_MSG_ID &&
        header->number_of_data_objects == 0u) {
  ...
  case PE_PRL_EVENT_MSG_RX_COMPLETE: { // done
    UCPD_MSG_HEADER *header = &pe_prl_cad->buffers[UCPD_RX_BUFFER_INDEX].header;

    if (header->number_of_data_objects != 0u ||
  ...
  case PE_PRL_EVENT_MESSAGE_FROM_PE: {
    ...
    if (pe_prl_cad->extended_message) {
      ...
      if (pe_prl_cad->chunked_rx_state ==
        ...
        UCPD_PRL_SM_CHUNKED_TX_Enter(
            pe_prl_cad, PRL_TCH_STATE_PREPARE_TO_SEND_CHUNKED_MESSAGE);
...
void UCPD_PRL_SM_RX_Enter(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  const UCPD_MSG *rx_msg = &pe_prl_cad->buffers[UCPD_RX_BUFFER_INDEX];
  ...
  case PRL_RX_STATE_CHECK_MESSAGE_ID: {
    if (pe_prl_cad->midStored == false ||
        pe_prl_cad->storedMID !=
            pe_prl_cad->buffers[UCPD_RX_BUFFER_INDEX].header.message_id)
...
void UCPD_PRL_SM_TX_Enter(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  case PRL_TX_STATE_CONSTRUCT_MESSAGE: {
    tx_msg->header.message_id =
        UCPD_CNT_Get(pe_prl_cad->port_number, UCPD_CNT_MESSAGEID);
  ...
  case PRL_TX_STATE_CHECK_RETRY_COUNTER: {
    ...
    if (UCPD_CNT_Get(pe_prl_cad->port_number, UCPD_CNT_RETRY) <=
        UCPD_CNT_N_RETRY)
      UCPD_PRL_SM_TX_Enter(pe_prl_cad, PRL_TX_STATE_CONSTRUCT_MESSAGE);
  ...
  case PRL_TX_STATE_MESSAGE_SENT: {
    ...
    if (pe_prl_cad->chunked_rx_state == PRL_RCH_STATE_REQUESTING_CHUNK)
      ...
      if (pe_prl_cad->lastChunk)
        ...
        UCPD_PRL_SM_CHUNKED_TX_Enter(pe_prl_cad,
...
void UCPD_PRL_SM_CHUNKED_RX_Enter(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  case PRL_RCH_STATE_PROCESSING_EXTENDED_MESSAGE: {
    ...
    if (rx_msg->ext_message.ext_header.chunk_number == 0u) {
      ...
      *ext_rx_msg = UCPD_MSG_INIT;
      ...
      ext_rx_msg->ext_message.ext_header = rx_msg->ext_message.ext_header;
    }

    if (rx_msg->ext_message.ext_header.chunk_number ==
        pe_prl_cad->chunk_number_expected) {

      // black magic, even gonna keep the curly braces

      uint8_t remaining = rx_msg->ext_message.ext_header.data_size -
      ...
      COPY_RAW(rx_msg->buffer + 4u,
               ext_rx_msg->buffer + pe_prl_cad->num_bytes_received + 4u,
               bytes_to_copy);
    ...
    if (pe_prl_cad->num_bytes_received >=
        ext_rx_msg->ext_message.ext_header.data_size) {
  ...
  case PRL_RCH_STATE_REQUESTING_CHUNK: {
    ...
    *tx_msg = UCPD_MSG_INIT;
    ...
    tx_msg->ext_message.ext_header = (UCPD_MSG_EXT_HEADER){
...
void UCPD_PRL_SM_CHUNKED_ROUTER_Enter(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  case PRL_RTR_STATE_RX_CHUNKS: {
    ...
    if (pe_prl_cad->chunked_rx_state != PRL_RCH_STATE_WAITING_CHUNK &&
      ...
      if (rx_msg->ext_message.ext_header.chunked == false)
        UCPD_PRL_SM_CHUNKED_RX_Enter(pe_prl_cad, PRL_RCH_STATE_REPORT_ERROR);
  ...
  case PRL_RTR_STATE_TX_CHUNKS: {
    ...
    else if (rx_msg->header.extended == true &&
             rx_msg->ext_message.ext_header.request_chunk == true) {
      if (rx_msg->ext_message.ext_header.chunk_number ==
...
void UCPD_PRL_SM_CHUNKED_TX_Enter(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  UCPD_MSG *tx_msg = &pe_prl_cad->buffers[UCPD_TX_BUFFER_INDEX];
  const UCPD_MSG *ext_tx_msg = &pe_prl_cad->buffers[UCPD_EXT_TX_BUFFER_INDEX];
  UCPD_MSG *ext_rx_msg = &pe_prl_cad->buffers[UCPD_EXT_RX_BUFFER_INDEX];
  ...
  case PRL_TCH_STATE_CONSTRUCT_CHUNKED_MESSAGE: {
    ...
    uint16_t total = ext_tx_msg->ext_message.ext_header.data_size;
    ...
    tx_msg->ext_message.ext_header = ext_tx_msg->ext_message.ext_header;

    tx_msg->header.number_of_data_objects = chunk_ndo;
    tx_msg->ext_message.ext_header.chunk_number =
        pe_prl_cad->chunk_number_to_send;

    // copy chunk payload from ext TX buffer into the start of body for TX
    COPY_RAW(((uint8_t *)&ext_tx_msg->ext_message.body) + bytes_sent,
             &tx_msg->ext_message.body, bytes_to_copy);
...
void UCPD_PRL_SM_HRD_Enter(UCPD_PE_PRL_CAD_Module *pe_prl_cad,
  ...
  case PRL_HR_STATE_REQUEST_HARD_RESET: {
    ...
    UCPD_PRL_SM_HRD_Enter(pe_prl_cad,
