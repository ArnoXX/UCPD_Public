...
void confTxDataTransfer(UCPD_HW_PORT_Handle* port_handle, uint8_t* buffer, uint32_t size)
{
	WRITE_REG(port_handle->txData->CPAR, (uint32_t)&(port_handle->instance->TXDR));
	WRITE_REG(port_handle->txData->CMAR, (uint32_t)buffer);
	WRITE_REG(port_handle->txData->CNDTR, size);
}

void confRxDataTransfer(UCPD_HW_PORT_Handle* port_handle, uint8_t* buffer, uint32_t size)
{
	WRITE_REG(port_handle->rxData->CPAR, (uint32_t)&(port_handle->instance->RXDR));
...
UCPD_PE_PRL_CAD_Event getCADEventFromCC(UCPD_PORT_Number port_number)
{
	...
	UCPD_TypeDef* instance = port_handles[port_number].instance;
	...
	if (UCPD_CC_DETECT_VOPEN(CC1_val, CC1) && !UCPD_CC_DETECT_VOPEN(CC2_val, CC2))
		port_handles[port_number].cc = UCPD_CC2;
...
void UCPD_PHY_SetCC(UCPD_PORT_Number port_number)
{
	LL_UCPD_SetCCPin(port_handles[port_number].instance,
	                 port_handles[port_number].cc == UCPD_CC1 ? LL_UCPD_CCPIN_CC1 : LL_UCPD_CCPIN_CC2);
}

void UCPD_PHY_EnableCC(UCPD_PORT_Number port_number, bool both)
{
	if (both)
		LL_UCPD_SetccEnable(port_handles[port_number].instance, LL_UCPD_CCENABLE_CC1CC2);
	else
		LL_UCPD_SetccEnable(port_handles[port_number].instance,
		                    port_handles[port_number].cc == UCPD_CC1 ? LL_UCPD_CCENABLE_CC1 : LL_UCPD_CCENABLE_CC2);
}
...
void UCPD_PHY_Init(UCPD_PORT_Number port_number)
{
	...
	*/
	...
	/* UCPD1_RX Init */
	LL_DMA_SetPeriphRequest(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMAMUX_REQ_UCPD1_RX);

	LL_DMA_SetDataTransferDirection(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);

	LL_DMA_SetChannelPriorityLevel(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_PRIORITY_LOW);

	LL_DMA_SetMode(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_MODE_NORMAL);

	LL_DMA_SetPeriphIncMode(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_PERIPH_NOINCREMENT);

	LL_DMA_SetMemoryIncMode(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_MEMORY_INCREMENT);

	LL_DMA_SetPeriphSize(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_PDATAALIGN_BYTE);

	LL_DMA_SetMemorySize(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_2, LL_DMA_MDATAALIGN_BYTE);

	/* UCPD1_TX Init */
	LL_DMA_SetPeriphRequest(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMAMUX_REQ_UCPD1_TX);

	LL_DMA_SetDataTransferDirection(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);

	LL_DMA_SetChannelPriorityLevel(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_PRIORITY_LOW);

	LL_DMA_SetMode(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_MODE_NORMAL);

	LL_DMA_SetPeriphIncMode(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_PERIPH_NOINCREMENT);

	LL_DMA_SetMemoryIncMode(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_MEMORY_INCREMENT);

	LL_DMA_SetPeriphSize(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_PDATAALIGN_BYTE);

	LL_DMA_SetMemorySize(UCPD_DATA_TRANSFER, LL_DMA_CHANNEL_3, LL_DMA_MDATAALIGN_BYTE);
	...
	port->pe_prl_cad.rx_buffer = &port_handles[port_number].buffers[port_handles[port_number].rx_buffer_index];
	port->pe_prl_cad.tx_buffer = &port_handles[port_number].buffers[UCPD_TX_BUFFER_INDEX];
	port->pe_prl_cad.ext_rx_buffer = &port_handles[port_number].ext_buffers[UCPD_EXT_RX_BUFFER_INDEX];
	port->pe_prl_cad.ext_tx_buffer = &port_handles[port_number].ext_buffers[UCPD_EXT_TX_BUFFER_INDEX];

	LL_UCPD_StructInit(&ucpd_settings);
	LL_UCPD_Init(port_handles[port_number].instance, &ucpd_settings);
	LL_UCPD_SetRxOrderSet(port_handles[port_number].instance, LL_UCPD_ORDERSET_SOP | LL_UCPD_ORDERSET_HARDRST);
...
void UCPD_PHY_Deinit(UCPD_PORT_Number port_number)
{
	...
	LL_UCPD_SetccEnable(port_handles[port_number].instance, LL_UCPD_CCENABLE_NONE);
}
...
void UCPD_PHY_SendMessage(UCPD_PORT_Number port_number)
{
	...
	LL_UCPD_WriteTxOrderSet(port_handles[port_number].instance, LL_UCPD_ORDERED_SET_SOP);
	LL_UCPD_SetTxMode(port_handles[port_number].instance, LL_UCPD_TXMODE_NORMAL);

	blockTxDataTransfer(&port_handles[port_number]);

	confTxDataTransfer(&port_handles[port_number], port_handles[port_number].buffers[UCPD_TX_BUFFER_INDEX].buffer,
	                   port->pe_prl_cad.txSize);

	unblockTxDataTransfer(&port_handles[port_number]);

	LL_UCPD_WriteTxPaySize(port_handles[port_number].instance, port->pe_prl_cad.txSize);
...
void UCPD_PHY_AssertRd(UCPD_PORT_Number port_number)
{
	...
	SET_BIT(SYSCFG->CFGR1,
	        (port_handles[port_number].instance == UCPD1) ? SYSCFG_CFGR1_UCPD1_STROBE : SYSCFG_CFGR1_UCPD2_STROBE);
...
void UCPD_PHY_Detach(UCPD_PORT_Number port_number)
{
	...
	CLEAR_STATUS(port_handles[port_number].instance->SR, port_handles[port_number].instance->SR);
...
void UCPD_PHY_Attach(UCPD_PORT_Number port_number)
{
	...
	confRxDataTransfer(&port_handles[port_number],
	                   port_handles[port_number].buffers[1u - port_handles[port_number].rx_buffer_index].buffer,
	                   UCPD_TOTAL_MSG_LEN);
	unblockRxDataTransfer(&port_handles[port_number]);

	confTxDataTransfer(&port_handles[port_number], port_handles[port_number].buffers[UCPD_TX_BUFFER_INDEX].buffer, 0);
...
void UCPD_PHY_HRDRS_Received(UCPD_PORT_Number port_number)
{
	...
	UCPD_INT_SET(port_handles[port_number].instance->IMR, UCPD_INT_PHY_WORKER_MASK);

	port->pe_prl_cad.hardResetActive = true;

	// disable tx
	UCPD_DisableTransmissionEvents(
	    port); // just in case this happens before the worker reaches the send message routine
...
void UCPD_PHY_RX_Complete(UCPD_PORT_Number port_number, bool error)
{
	...
	confRxDataTransfer(&port_handles[port_number],
	                   port_handles[port_number].buffers[port_handles[port_number].rx_buffer_index].buffer,
	                   UCPD_MAX_MSG_LEN);

	port_handles[port_number].rx_buffer_index = 1u - port_handles[port_number].rx_buffer_index;
	unblockRxDataTransfer(&port_handles[port_number]);
}
...
void UCPD_PHY_EnterErrorRecovery(UCPD_PORT_Number port_number)
{
	...
	LL_UCPD_SetRpResistor(port_handles[port_number].instance, LL_UCPD_RESISTOR_NONE);
	LL_UCPD_SetccEnable(port_handles[port_number].instance, LL_UCPD_CCENABLE_NONE);
	SET_BIT(SYSCFG->CFGR1,
	        (port_handles[port_number].instance == UCPD1) ? SYSCFG_CFGR1_UCPD1_STROBE : SYSCFG_CFGR1_UCPD2_STROBE);
	LL_UCPD_RxDisable(port_handles[port_number].instance);
...
void UCPD_Worker(UCPD_PORT_Number port_number)
{
	...
	pe_prl->rx_buffer = &port_handles[port_number].buffers[port_handles[port_number].rx_buffer_index];
	pe_prl->tx_buffer = &port_handles[port_number].buffers[UCPD_TX_BUFFER_INDEX];
	pe_prl->ext_rx_buffer = &port_handles[port_number].ext_buffers[UCPD_EXT_RX_BUFFER_INDEX];
	pe_prl->ext_tx_buffer = &port_handles[port_number].ext_buffers[UCPD_EXT_TX_BUFFER_INDEX];
...
void UCPD_PHY_Handler(UCPD_PORT_Number port_number)
{
	...
	if ((port_handles[port_number].instance->IMR & status) == 0u) return;

	/* Type C events */
	if (CHECK_STATUS(status, UCPD_SR_TYPECEVT1) || CHECK_STATUS(status, UCPD_SR_TYPECEVT2))
	{
	...
	if (CHECK_STATUS(status, UCPD_SR_RXMSGEND))
	{
		...
		if (error == false) UCPD_PD_PHY_Event_Handle(port_number, PHY_EVENT_MSG_RX_COMPLETE);
...
void UCPD_HW_Timer_Init(UCPD_PORT_Number port_number)
{
	...
	LL_TIM_OC_SetMode(UCPD_TIMER_INSTANCE, LL_TIM_CHANNEL_CH1, LL_TIM_OCMODE_FROZEN);
	LL_TIM_CC_DisableChannel(UCPD_TIMER_INSTANCE,
	                         LL_TIM_CHANNEL_CH1); // ensure off until used
	LL_TIM_OC_SetMode(UCPD_TIMER_INSTANCE, LL_TIM_CHANNEL_CH2, LL_TIM_OCMODE_FROZEN);
	LL_TIM_CC_DisableChannel(UCPD_TIMER_INSTANCE, LL_TIM_CHANNEL_CH2);
	LL_TIM_OC_SetMode(UCPD_TIMER_INSTANCE, LL_TIM_CHANNEL_CH3, LL_TIM_OCMODE_FROZEN);
	LL_TIM_CC_DisableChannel(UCPD_TIMER_INSTANCE, LL_TIM_CHANNEL_CH3);
	LL_TIM_OC_SetMode(UCPD_TIMER_INSTANCE, LL_TIM_CHANNEL_CH4, LL_TIM_OCMODE_FROZEN);
...
void UCPD_HW_PHY_EVENT_ISR(void)
{
	UCPD_PHY_Handler(0);
}

void UCPD_PE_PRL_WORKER_ISR(void)
{
	UCPD_Worker(0);
}

void UCPD_HRD_HELPER_ISR(void)
{
	UCPD_ISR_HRD_Helper(0);
}

void UCPD_TIMER_ISR(void)
{
	UCPD_TIM_Handler(0);
}
